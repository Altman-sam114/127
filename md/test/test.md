...
> 当前规则：默认云端重验证，本机不主动运行测试或检查命令。GitHub Actions 默认执行静态检查、通用 iOS build 和 `WWIIHexV0Probes` simulator probe；完整 XCTest、Smoke、Stage Regression、Dynamic Theater Regression、Full、UI / 截图和性能验证仍不在默认本机范围内。
...
- 云端 `xcodebuild build`
- 云端 simulator 上执行 `WWIIHexV0Probes` probe target
...
当前默认云端 workflow 只跑 `WWIIHexV0Probes` 作为轻量运行时 probe，不跑完整 `WWIIHexV0Tests`、Smoke、Stage Regression、Dynamic Theater Regression、Full、UI test、截图或性能检查；这些结果统一通过 JUnit `ci.tests` skipped 摘要说明，或不列入默认结果。
...
- `probe-xctest.log`：`WWIIHexV0Probes` 云端 simulator probe 日志。
