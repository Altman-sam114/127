...
- 云端 `xcodebuild build`
- 云端 simulator 上执行 `WWIIHexV0Probes` probe target，其中包含灰潮默认剧本的 10 个 AI 半回合轻量运行时 probe，以及 Playtest 红/蓝新局、Action Gate、主目标摘要和本地快照状态 probe
...
当前默认云端 workflow 只跑 `WWIIHexV0Probes` 作为轻量运行时 probe。该 probe 可作为默认灰潮数据加载、AI 半回合推进、`end_turn` 规则链路、战略派生层刷新、Playtest 红/蓝新局状态、Action Gate / 主目标摘要和本地快照 envelope 恢复的 cloud Probe 证据；它不等于真实 App observer 模式长跑，也不跑完整 `WWIIHexV0Tests`、Smoke、Stage Regression、Dynamic Theater Regression、Full、UI test、截图或性能检查；这些结果统一通过 JUnit `ci.tests` skipped 摘要说明，或不列入默认结果。
...
- `probe-xctest.log`：`WWIIHexV0Probes` 云端 simulator probe 日志，需核对灰潮 10 个 AI 半回合 probe 和 Playtest 状态 probe 用例执行并通过。
