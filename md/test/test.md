...
- 云端 `xcodebuild build`
- 云端 simulator 上执行 `WWIIHexV0Probes` probe target，其中包含灰潮默认剧本的 10 个 AI 半回合轻量运行时 probe
...
- `probe-xctest.log`：`WWIIHexV0Probes` 云端 simulator probe 日志，需核对灰潮 10 个 AI 半回合 probe 用例执行并通过。
