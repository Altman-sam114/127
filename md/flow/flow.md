...
- `UnitInspectorView` / `RegionInspectorView` 对 legacy raw id 派生显示增加 compatibility scrub：旧作战区、旧地名或旧单位 raw id 若进入兼容路径，会显示为 `Compatibility Area`，不再把主要旧题材 token 直接标题化给玩家。
