...
- `AgentPanelView` 的折叠 Technical Replay 仍保留 raw JSON / raw id 审计能力；普通主视图已做现代显示清洗，但 debug-style replay 可能显示 schema key 或 legacy id，发布前若面向普通玩家应考虑增加脱敏或 debug-only 开关。
