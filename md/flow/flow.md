...
- `UnitInspectorView` / `RegionInspectorView` 对 legacy raw id 和 fallback 区域显示增加 compatibility scrub：旧作战区、旧地名、旧单位 raw id、旧编辑器生成的 `City q,r` / `新省份*` 名称若进入兼容路径，会显示为 `Compatibility Area`；`RegionInspectorView` 的区域标题、City 和 Objectives，以及 Mission target summary / 选区反馈也复用同类显示清洗，不再把主要旧题材 token 直接标题化给玩家。
- `AgentPanelView` 普通主视图已做现代显示清洗，且默认不显示 Technical Replay / raw JSON；`AgentDecisionRecord.rawJSON` 仍在记录层保留 raw JSON / raw id 审计能力。fallback 错误文本、diagnostics 或未来重新开启的技术复盘仍可能显示 schema key 或 legacy id，后续应继续做显示层脱敏和显式审计入口。
