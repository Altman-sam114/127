...
MapEditor / JSON 数据
  ...
  -> FrontLine 动态 hex 接触
  -> WarDeployment hexToFrontZone + FRONT/DEPTH/GARRISON internal roles
     (UI display: Contact Line / Reserve / Security)
  ...
  -> ModernSubDirectiveCommandCompiler (limited ISR Recon Area + Fires Fire Mission + EW Electronic Warfare + Air Tasking SEAD bridge)
...
- `ModernCommandChainPlan` 主要做可审计分解、JSON 校验和复盘展示；当前 `ISR Coordinator / Recon Area`、`Fires Coordinator / Fire Mission`、`EW Coordinator / Electronic Warfare` 与 `Air Tasking / Suppress Air Defense` 有受限执行桥，会在 ZoneDirective 执行前最多编译并执行 1 条 `Command`：ISR 生成 `Command.recon` 并经 `CommandValidator -> RuleEngine -> VisibilityRules` 刷新 contact 和 intelligence event；Fires 仅在存在 contact track 时生成 `Command.fireMission`，并继续经 `CommandValidator -> RuleEngine -> FireSupportRules` 校验弹药、冷却、目标质量、ROE、防空和友邻风险；EW 仅从当前 active faction、指定 zone 内、未行动且 EW 组件权重达标的 formation 中选 1 个候选，生成 `Command.electronicWarfare` 并经 `CommandValidator -> RuleEngine -> VisibilityRules` 写入 EW effect、刷新 awareness 和电子战日志；Air Tasking 目前只桥接 SEAD，且只接受当前 active faction 可见的 medium+ 新鲜 hostile air-defense contact，选择当前 active faction、指定 zone 内、未行动且具备火力 / 无人 / EW / 防空压制能力的 formation，生成 `Command.suppressAirDefense` 并经 `CommandValidator -> RuleEngine -> FireSupportRules` 消耗弹药、设置冷却、写入 suppression effect 和 fireSupport 日志，禁止无 contact 的区域盲压制。执行结果写入 `AgentDecisionRecord.commandResults`。Logistics / Brigade sub-directive 仍为 advisory / ZoneDirective 路径，不直接执行。
...
- 统治者层只作为后续方向预留；当前执行主链路不调用 `RulerAgent`，也不写统治者决策记录；预留 `RulerAgent` 的敌情估算已改为读取 visible contacts，不直接读取不可见敌军真实 strength。
...
人工提出目标
  ...
  -> Agent C 核对 manifest / junit / xcodebuild.log / probe-xctest.log / simulator.log / failure summary
...
旧 `tank`、`motorizedInfantry`、`infantry`、`artillery` component raw value 仍可解码，用于旧 JSON、旧测试和 fallback。`modern_unit_templates.json` 当前只保留现代 template id；旧 `*_division` template id 兼容留在 `unit_templates.json` 和 `DataLoader` fallback，玩家默认路径不再通过旧模板 id 生成灰潮单位。
...
- 地面直接攻击和火力任务的 contact gate 分层：默认现代剧本中的 direct attack 需要 medium+ visible linked contact；restricted / civilian 区的 precision / loitering 火力需要 current high-confidence linked hostile contact。
...
- 现代 sub-directive 默认仍是 advisory；当前只有 ISR Recon Area、Fires Fire Mission、EW Electronic Warfare 和 Air Tasking Suppress Air Defense 四条受限 bridge 会各自最多编译 1 条 `Command`，且仍必须经 `CommandValidator -> RuleEngine` 执行，不能直接改 `GameState`。
...
- sub-directive 还没有独立调参 UI；当前仅 ISR Recon Area、Fires Fire Mission、EW Electronic Warfare 和 Air Tasking Suppress Air Defense 四条最小路径会受限编译成 `Command`，Logistics / Brigade 仍未接入直接执行桥。
- ChiefOfStaff 当前是 deterministic notes / deconflict 说明，未做复杂冲突仲裁搜索。

## 0.9 v6.7 玩家现代指挥 UI、任务计划和人机协同

v6.7 第一批实现把玩家侧从单纯 `Hold / Withdrawal / Sustain` 扩展为现代任务面板。它不新增旁路执行器，不在 SwiftUI View 里改状态，而是把任务按钮接到 `AppContainer`，再由现有 `Command` 或 `ZoneDirective` 管线执行。
...
- `FireMissionTarget.contextualDisplayName(in:)` 对 region 优先使用当前 map 的 region name，但识别旧地名、旧作战 token、旧编辑器生成 `City q,r` / `新省份*` 时显示为 `Objective Area`；contact 缺上下文时只显示 `Contact Track` / `Unknown Contact`，fire mission 失败叙事也走 contextual display，避免事件日志从 raw contact / region id 派生玩家可见目标名。
...
- `AgentPanelView` 对 provider、agent/context、command-chain objective / contact 目标和 mapping error 做显示层收口：未知 provider 只显示 Local / External Planner，未知 `MockAI+...` suffix 显示为 planner extension，agent/context 中的 generated zone id 回退为 Command Planner，command-chain target 行使用 objective area / contact track，不直接拼 raw region / contact id；`AgentCommandMapper` 缺字段错误改用 destination objective / target formation 玩家语义。底层 provider id、agent id、schema key、raw JSON 和 directive record 仍保留在审计数据中。
...
- `playtestObjectiveSummaryText` 与 `playtestObjectiveThresholdText` 从 `VictoryRules.greyTideObjectiveControlCounts(in:)` 派生，显示 Blue / Red / Neutral 对十个主目标的控制数；Blue 玩家看到 Blue 胜利阈值，Red 玩家看到阻止 Blue 达标的阈值口径。它只读 `GameState`，不写胜负状态；胜负状态由 `RuleEngine -> CommandExecutor -> VictoryRules` 在 Grey Tide 移动占领主目标后刷新。
...
- Observer AI 开关本身只改变容器状态，不直接写游戏状态；当用户通过 End Turn / `advanceOrRunAI()` 推进时，`AppContainer.runAISequence` 在 observer 模式最多连续解析两个 AI 半回合，从 Blue 到 Red 再回到 Blue command phase。
...
- AI 面板空态、Commander / Unit / Region / Economy 面板继续收口玩家可见旧口径：`guderian` / `MockAI` 占位和 provider 组合名改为现代 planner 文案，默认 `generals.json` 改为灰潮虚构 Blue / Red commander，`GeneralDispatcher` 的 seed 匹配会排除已分配 commander，ruler / theater / front zone / division / IC / supplies 标签改为 national command / operational zone / command sector / formation / contact line / personnel / materiel / logistics。
- 灰潮默认数据生成 `TheaterNode` 时会把 `TheaterId.rawValue` 格式化为玩家可读的 command sector 名称；后续 `WarDeploymentManager` 生成 `FrontZone.name` 时继承该显示名。`TheaterId` / `FrontZoneId` / `regionToTheater` / `hexToTheater` / `hexToFrontZone` 仍是运行和映射权威，不因显示名改变。
...
- 灰潮默认剧本扩展到 120 hex / 30 region / 16 初始 formation / 25 objectives，并通过十个主目标接入现代胜负判断；地面移动占领 neutral 主目标会刷新十个主目标控制数，达到 7 个 Blue 主目标时立即写入 Grey Tide 胜利状态；默认现代模板资源保留 10 个现代 template id，旧 `*_division` template id 只作 legacy 数据 / fallback 兼容。
...
- 若灰潮现代资源加载失败，`DataLoader.loadInitialGameState()` 仍会进入旧阿登或 `GameState.initial()` compatibility fallback；该路径可能暴露旧单位名、旧 agent 或旧胜负语义，属于发布候选 residual risk，不是默认成功加载路径。
- `MapEditorExporter.export` 普通独立导出不携带灰潮 RC 的十个主目标胜负门、`mainObjective` 和高级胜负条件；只有通过 `MapEditorGameResourceBridge.overwriteDefaultGameResources` 覆盖默认灰潮资源时，才会回填既有 maxTurns、victoryConditions、VP、occupationState、river crossing 和 mainObjective 等高级元数据。
- `UnitInspectorView` / `RegionInspectorView` 对 legacy raw id 和 fallback 区域显示增加玩家可见清洗：旧作战区、旧地名、旧单位 raw id、旧编辑器生成的 `City q,r` / `新省份*` 名称若进入兼容路径，会按上下文显示为 `Objective Area`、`Zone Area` 或 `Sector Area`；`RegionInspectorView` 的区域标题、City 和 Objectives，以及 Mission target summary / fire mission target display / 选区反馈也复用同类显示清洗，不再把主要旧题材 token 或 compatibility 术语直接标题化给玩家。
- `AgentPanelView` 普通主视图已做现代显示清洗，且默认不显示 Technical Replay / raw JSON；本轮进一步把未知 provider、agent/context generated id、command-chain objective / contact target 和 mapping error 收口为 planner / objective area / contact track / destination objective / target formation 文案。`AgentDecisionRecord.rawJSON` 仍在记录层保留 raw JSON / raw id 审计能力。fallback diagnostics 或未来重新开启的技术复盘仍可能显示 schema key 或 legacy id，后续应继续做显示层脱敏和显式审计入口。

发布前仍需人工授权：

- 真实 App iOS Simulator 或真机启动、macOS target 启动。
- 基础 UI 点击烟测、SpriteKit 截图或人工视觉检查。
- App 图标在 Dock / SpringBoard / 安装包中的真实显示。
- 真实 App 至少 10-20 回合 observer 模式和性能体感检查；云端 10 个 AI 半回合 Probe、Playtest 状态 Probe、Observer AI 容器入口 Probe 与 restricted / civilian fire ROE Probe 不能替代该发布前运行时验收。
...
`TheaterNode.name` 是展示字段。灰潮默认数据由 `DataLoader.makeTheaterState` 从 `TheaterId.rawValue` 派生可读名称，例如把下划线 id 转为标题式 command sector 名称；`TheaterId` 本身仍是 JSON、Codable、`regionToTheater` 和 `hexToTheater` 的权威键。
...
- `FrontZone.name` 是展示字段，初始值来自对应 `TheaterNode.name`，不得当作映射键。
...
检查规范以 `md/test/test.md` 为准。当前默认不在本机跑 Xcode / XCTest / 模拟器 / 性能类验证；云端默认执行静态检查、通用 iOS build 和 `WWIIHexV0Probes` simulator probe，且 Probe 覆盖灰潮 10 个 AI 半回合轻量运行时链路、Playtest 状态链路、neutral 主目标地面占领到即时胜利阈值闭环、Observer AI 容器入口链路、restricted / civilian fire ROE 规则入口和 direct attack contact gate；完整 XCTest / UI / 截图 / 真实 App observer 长回合仍跳过。
...
- Probe：`WWIIHexV0Probes`
  ...
  - v6.10 neutral 主目标地面占领到即时胜利阈值：移动占领第 7 个 Blue 主目标后刷新 `VictoryRules`、Playtest 目标摘要、Action Gate 和阈值文案。
...
GameState / WarDirectiveRecord / EventLog
  -> RootGameView
  -> HUD + Info tabs
  -> AgentPanelView 展示 command chain / command results；raw JSON 保留在记录层
...
- AI 面板从只展示 `AgentDecisionRecord` 扩展为同时展示 `WarDirectiveRecord`；默认主视图显示结构化 command chain、command results 和 errors，完整 directive 明细、diagnostics 与 raw JSON 保留在记录层，普通玩家面板默认不显示 Technical Replay。
...
- AI：结构化 command chain、记录层 raw JSON、directive diagnostics 是否能解释 AI 回合，且普通玩家面板不暴露 Technical Replay。
