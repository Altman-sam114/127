...
- 没有多步骤新局向导或 AI 控制矩阵；当前只支持 Playtest tab 中的 Blue / Red 新局选择与 observer mode。
...
- Playtest 面板提供 `New Operation Side` segmented control，可用 Blue Force 或 Red Force 新开灰潮局；状态区继续显示 `Player Side`、`Opposition` 和 `Control Mode`，对手由当前局面 hostile faction 推导。
