...
- `FireMissionTarget.contextualDisplayName(in:)` 对 region 优先使用当前 map 的 region name，但识别旧地名、旧作战 token、旧编辑器生成 `City q,r` / `新省份*` 时显示为 `Objective Compatibility Area`；contact 缺上下文时只显示 `Contact Track` / `Unknown Contact`，fire mission 失败叙事也走 contextual display，避免事件日志从 raw contact / region id 派生玩家可见目标名。
...
- `UnitInspectorView` / `RegionInspectorView` 对 legacy raw id 和 fallback 区域显示增加 compatibility scrub：旧作战区、旧地名、旧单位 raw id、旧编辑器生成的 `City q,r` / `新省份*` 名称若进入兼容路径，会显示为 `Compatibility Area`；`RegionInspectorView` 的区域标题、City 和 Objectives，以及 Mission target summary / fire mission target display / 选区反馈也复用同类显示清洗，不再把主要旧题材 token 直接标题化给玩家。
