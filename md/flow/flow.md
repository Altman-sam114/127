...
- 真实 App 至少 10-20 回合 observer 模式和性能体感检查；云端 10 个 AI 半回合 Probe、Playtest 状态 Probe 与 Observer AI 容器入口 Probe 不能替代该发布前运行时验收。
...
检查规范以 `md/test/test.md` 为准。当前默认不在本机跑 Xcode / XCTest / 模拟器 / 性能类验证；云端默认执行静态检查、通用 iOS build 和 `WWIIHexV0Probes` simulator probe，且 Probe 覆盖灰潮 10 个 AI 半回合轻量运行时链路、Playtest 状态链路和 Observer AI 容器入口链路；完整 XCTest / UI / 截图 / 真实 App observer 长回合仍跳过。
