...
- 灰潮默认剧本扩展到 120 hex / 30 region / 16 初始 formation / 25 objectives，并通过十个主目标接入现代胜负判断；默认现代模板资源保留 10 个现代 template id，旧 `*_division` template id 只作 legacy 数据 / fallback 兼容。
