...
final class AgentPipelineTests: XCTestCase {
    ...
    func testParserRejectsMismatchedSchemaAgentAndTurn() {
        // v0.2: parser 默认支持 schemaVersion 1 (hex) + 2 (region)。3 应拒绝。
        let parser = AgentDecisionParser()
        let schemaJSON = """
        {"schemaVersion":3,"agentId":"guderian","turn":1,"intent":"x","orders":[]}
        """
        let agentJSON = """
    ...
    func testProviderFailureDoesNotModifyGameState() async {
        ...
        let outcome = await manager.runGermanAITurn(state: state, pipelineMode: .legacyAgentOrder)
