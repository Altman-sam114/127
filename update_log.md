...
- 2026-07-06：继续收紧 v6.10 现代玩家可见文本防回归脚本。`scripts/check_modern_visible_text.rb` 不再整文件排除 `AgentConfiguration.swift` 和 `AgentPromptBuilder.swift`，而是把 `guderian`、`ger_panzer_1`、`targetDivisionId` 等必要 legacy id / schema key 收窄为精确 allowlist；README、`md/test/test.md`、v6.10 progress / evidence 同步记录该检查范围变化。本轮不改变 `AgentDecisionRecord`、legacy JSON schema、AI provider id、Codable 或 `Command` / `ZoneDirective` / `WarCommandExecutor` / `RuleEngine` 执行链路；未跑本机 Ruby / Xcode / XCTest / Probe / simulator 检查，等待 GitHub Actions artifact 验证。
