...
- 2026-07-05：继续推进 v6.10 本地快照 schema 兼容。`AppContainer.saveLocalSnapshot()` 从裸 `GameState` JSON 改为保存 `LocalPlaytestSnapshot` envelope，包含 schemaVersion、savedAt、playerFaction 和 gameState；`loadLocalSnapshot()` 先恢复 envelope，失败时兼容旧裸 `GameState` 快照并使用旧 playerFaction key 或默认作战方推断玩家方。默认灰潮 JSON 资源仍不被保存/继续流程写回。同步更新 README、flow、v6.10 progress/evidence。仍未跑本机 Xcode / 模拟器 / UI 点击 / 长回合 observer，原因是当前规范未授权本机重测试。
