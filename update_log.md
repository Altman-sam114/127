...
- 2026-07-06：继续加深 v6.10 云端 Probe 覆盖。`WWIIHexV0/Probes/WWIIHexV0ProbeTests.swift` 新增灰潮默认剧本 10 个 AI 半回合轻量运行时 probe，直接使用 `TurnManager.runAITurn(..., pipelineMode: .marshalDirective)` 轮流推进 active Blue / Red faction，检查 phase 与 active faction 对齐、`end_turn` 执行、至少一个 directive command 被执行、无 outside controllable phase / no directives 错误、10 个半回合精确推进 5 个完整 turn，并复核 region graph、command sector、theater state、sensor coverage 和运行期间 visible contacts。README、`md/test/test.md`、`md/flow/flow.md`、`md/flow/flowchart.md`、v6.10 progress/evidence 同步说明该 cloud Probe 可作为默认灰潮数据加载、AI 半回合推进、指令编译和规则管线的云端证据；artifact 应核对 `probeOutcome=success`、`simulatorOutcome=success` 和 `probe-xctest.log` 中对应 probe 通过。该 Probe 不等于真实 App UI 启动、SpriteKit 截图、完整 XCTest / Smoke / Stage / Full、真实 App 10-20 回合 observer 长跑或性能体感检查。本轮未跑本机 Xcode / XCTest / Probe / simulator / UI / 截图 / 性能，按用户要求只等待 GitHub Actions artifact 验证。
