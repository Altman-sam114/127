...
> **当前状态：v6.10 发布候选准备。工程底座仍来自 WWIIHexV0，源码兼容名、target/module 名和旧阿登 fallback 数据仍保留；已完成 v6.0-v6.9 的现代作战方、灰潮种子、现代单位、ISR/EW/contact、火力/空中任务、现代 AI 指挥链、玩家任务入口、现代 C2 UI 和试玩闭环首轮。本轮把主游戏 iOS / macOS display name 收口为 `Modern Command Agent`，新增现代 C2 AppIcon 资产，Playtest 面板支持红/蓝新局选择并显示 Player Side / Opposition / Control Mode / Action Gate / 十个主目标控制摘要，任务面板显示 Mission Status 并按 Recon、UAV、Fire Mission、SEAD、EW 和 Resupply 各自的 validator 预检启用按钮，Playtest 命令反馈区分 success / warning / failure tone，HUD、任务、单位、外交和事件日志主路径使用 Logistics / sustainment 与现代 display 文案，默认 commander 数据改为虚构 Blue / Red C2 staff，commander seed 会排除已分配 commander，Commander / Unit / Diplomacy / MapEditor 默认导出继续减少 raw id 和旧 supply 口径暴露，地图补给源标签收口为 `LOG B` / `LOG R` 后勤口径，灰潮默认剧本扩到 120 hex / 30 region，并加入现代目标控制胜负判断；同时新增发布候选残留扫描、验收证据矩阵与人工授权重验证清单。玩家任务仍经 `AppContainer -> Command / ZoneDirective -> WarCommandExecutor / RuleEngine`，Mission Status 会复用 validator 预检列出 Ready Tasks 或解释弹药、冷却、目标质量、ROE、防空、目标缺失和友邻风险等拒绝原因；默认现代剧本中的 direct attack 也要求 medium+ visible linked contact，避免猜 id 攻击绕过迷雾；Grey Tide 中地面移动占领第 7 个主目标会立即刷新 `VictoryRules` 胜负状态，让 Playtest 目标摘要、Action Gate 和胜利阈值文案同步闭环；Allow Retreat 的玩家可见命令文案收口为 withdrawal，不再把 fallback 作为普通命令反馈。玩家、AI 和 directive 回放的规则拒绝反馈使用可读文案和 tone，不再把 enum raw value 暴露给玩家。AI 面板默认展示结构化 command chain、command results 和 errors，其中 ISR Coordinator / Recon Area、Fires Coordinator / Fire Mission、EW Coordinator / Electronic Warfare 已有受限执行桥，本轮继续新增 Air Tasking / Suppress Air Defense 受限 `Command.suppressAirDefense` 执行桥；四者均经 `ModernSubDirectiveCommandCompiler -> CommandValidator -> RuleEngine` 进入既有 VisibilityRules / FireSupportRules，且 SEAD bridge 只针对当前 active faction 可见的 medium+ 新鲜 air-defense contact 选择 1 个可行动 formation。provider、agent/context、objective/contact 目标行和 mapping error 进一步收口为 Local/External Planner、Command Planner、objective area / contact track 等中性文案；底层 `AgentDecisionRecord.rawJSON` 继续保留完整 directive JSON 供审计，但普通玩家面板默认不显示 Technical Replay / raw JSON。Fire mission 失败叙事和目标 fallback 也改用上下文显示或中性 Contact Track / Objective Area，避免旧 contact / region raw id 进入事件日志。历史测试基线曾达到 v0.37 Probe 18/0、Stage Regression 69/0、Full 226/0；当前工作流默认不在本机跑测试或检查命令，静态检查、通用 iOS build 和 `WWIIHexV0Probes` 云端 simulator probe 由 GitHub Actions artifact 验证，其中 Probe 覆盖灰潮 10 个 AI 半回合轻量运行时链路，并覆盖 Playtest 红/蓝新局、Action Gate、主目标摘要、本地快照状态链路、neutral 主目标地面占领到即时胜利阈值的闭环、Modern Mission Recon 容器链路、Modern Command Chain ISR Recon Area、Fires Fire Mission、EW Electronic Warfare 与本轮新增 Air Tasking Suppress Air Defense 受限执行桥、Observer AI 容器入口自动推进、restricted / civilian fire ROE 规则入口和 direct attack contact gate；restricted 区精确火力现在还要求 current high-confidence linked hostile contact，stale 或 medium contact 不放行。上一条 EW bridge 已随 `20be64d/28854943054` 云端 artifact 复核通过，`WWIIHexV0Probes` 为 28/0；本轮 SEAD bridge 等待最新 `origin/main` Actions artifact 复核。**

---

## 协作与云端验证

当前协作流程固定为 `main` 直推：Agent B 默认不在本机跑测试或检查命令，commit 后 push 到 `origin/main` 触发 GitHub Actions；Agent C 通过未加密 CI 结果包核对 `ci-artifact-manifest.json`、`junit.xml`、`xcodebuild.log`、`probe-xctest.log`、`simulator.log`、`grey-tide-data.log`、`modern-visible-text.log` 和失败摘要。详细规则见 `AGENTS.md`、`md/test/test.md`、`md/prompt/README.md`。

灰潮默认剧本的静态数据一致性默认由 GitHub Actions 执行；人工明确授权本机检查时，可用 `ruby scripts/check_grey_tide_data.rb` 复查。该脚本只读取 JSON 和 `VictoryRules.swift`，核对 tile / region / objective / unit template 引用、objective 与 key location 一一映射、region edges 与 neighbors 一致、补给源 faction / controller 对齐、初始单位不落敌控区，并确认 VictoryRules、scenario victoryConditions 和 region `mainObjective` 的十个主目标集合一致；同时强制 v6.10 RC 规模为 120 tiles / 30 regions / 16 initial units / 25 objectives / 10 main objectives / 10 modern templates，不启动 app。主路径玩家可见现代文案同样默认由云端 `ruby scripts/check_modern_visible_text.rb` 复查；人工授权本机检查时可本机运行。该脚本扫描 App / UI / SpriteKit 字符串、命令结果 / 日志 / diagnostics / legacy prompt 字符串、Core 可见 displayName 映射和默认现代 JSON 显示字段，防止 WWII / Ardennes / Panzer / Guderian / Germany / Allies / SUP A / SUP G 等旧口径重新进入默认现代 UI 或数据；`AgentConfiguration.swift` 和 `AgentPromptBuilder.swift` 只保留精确 legacy id / schema key allowlist。
...
当前 v6.10 是发布候选准备态，不是正式发布。`grey_tide_2030` 目前是 120-hex / 30-region 的可加载现代发布候选剧本，用于替换默认阿登入口并验证现代数据链；它包含机场、港口、雷达/防空、河桥、铁路、燃料、通信、民用缓冲和蓝/红补给入口，默认 `initialPhase` 使用现代 `blueCommand` alias，旧 `alliedPlayer` / `germanAI` phase 值仅作 legacy decode 兼容；`VictoryRules` / `RegionVictoryRules` 对 `grey_tide_2030` 使用十个主目标的现代目标控制判定，Civilian Evac Zone 等 key-only 地点不计入十个主目标；地面移动占领 neutral 主目标后会通过 `RuleEngine -> CommandExecutor -> VictoryRules` 刷新目标控制数和 Grey Tide 即时胜负状态；`scripts/check_grey_tide_data.rb` 会校验 VictoryRules 硬编码集合、scenario victoryConditions 和 region `mainObjective=true` 三套来源一致，并额外检查 objective/key location、region edge/neighbor、补给源、初始 phase、初始单位落点和 v6.10 RC 数据规模的静态语义一致性。MapEditor 默认桥接读写灰潮资源，编辑器可见模式收口为区域、作战区和任务编组；覆盖默认资源时会保留灰潮已有 `maxTurns`、胜负条件、河流边、VP、occupation 和 river crossing 等编辑器未表达的高级元数据；普通 `MapEditorExporter.export` 独立导出不会自动携带灰潮 RC 十个主目标胜负门，导出 dataNotes 已写明需通过 game resource bridge 覆盖默认行动才能保留这些高级元数据。`modern_unit_templates.json` 已提供现代组件并通过现有 `strength + supplyState + components` 影响移动、战斗和补员成本，灰潮剧本未知模板不再静默回退为 infantry。玩家任务面板可以发起 Recon Area、UAV Orbit、Fire Mission、Air Support / SEAD、Assault Objective、Hold / Delay、Sustain / Repair、Jam / Counter-Drone；实际行动仍由 `Command` 或 `ZoneDirective` 进入 `RuleEngine`。任务面板会显示 Mission Status 来解释按钮不可用原因，Recon / UAV / Fire Mission / SEAD / EW / Resupply 按钮分别复用 `CommandValidator` 预检当前 formation、target、munition 和规则状态；Mission Status 会列出可用 Ready Tasks，或显示首个可读阻塞原因，避免 Fire Mission 缺目标遮住可用的侦察、EW 或后勤任务；任务面板、单位详情和单位 tooltip 现在从现有 `strength + supplyState + hasActed + retreat + components` 派生显示 Readiness、Fuel posture 和 Signature posture，不新增 Codable 字段，也不把它们作为新的规则门禁。Fire Mission 只使用选中 hex / region 命中的 visible contact、选中 region 或选中 hex，不再无选择地回退到第一个 visible contact；默认现代剧本的 direct attack 同样要求 medium+ visible linked contact，本轮新增 cloud Probe 锁定无 contact 拒绝、有 medium linked contact 放行。`UnitInspectorView` / `RegionInspectorView` 会把兼容路径中的旧 raw id token 按上下文显示为 `Objective Area`、`Zone Area` 或 `Sector Area`，避免旧快照或旧 fallback 被标题化成主要玩家文案。在 neutral / green / co-belligerent 控制区内，tube / rocket area fires 会被 `restrictedFireZone` 拒绝，precision / loitering 只有解析到 current high-confidence linked hostile contact 时才允许并以 restricted fire zone 风险降级结算；stale 或 medium contact 在 restricted 区不会放行。拒绝反馈会进入 `lastCommandMessage` 和 interaction log，Allow Retreat 反馈使用 withdrawal 口径，Playtest 面板按 success / warning / failure tone 显示图标、颜色和 VoiceOver 标签，Field Prompts 只对 Ready Tasks 使用完成态图标，主 UI 后勤文案使用 Logistics 口径。Playtest tab 支持选择 Blue Force 或 Red Force 新开灰潮局、保存/继续本地快照、清除快照、切换 observer AI、地图图层和 C2 overlay 显示；本地快照使用带 schemaVersion 的 envelope 保存 `GameState` 与玩家方，新保存的 phase 写 `blueCommand` / `redCommand`，并保留旧裸 `GameState` 快照兼容读取。Playtest 状态区显示 Player Side、Opposition、Control Mode、Action Gate、十个主目标控制摘要、不遮挡地图的短引导和错误反馈；Action Gate 只读解释当前是哪个阵营可下令、AI 可解析、observer 自动化、需要结束回合，还是胜利已达成，红方玩家会看到阻止 Blue 达标的目标阈值口径，不直接执行命令。非 observer 模式下 AI 只接管当前非玩家敌对阵营，observer 模式下 `AppContainer.advanceOrRunAI()` 会经容器入口连续解析最多两个 AI 半回合。fuel / readiness / signature 已有派生态势显示，但独立持久字段、真实燃料消耗、真实武器库、复杂实时空战和真本地 LLM 多 Agent 并发仍未独立建模。`GamePhase.germanAI/alliedPlayer`、`Division` 源码名、旧 unit template id、target/module 名和若干二战测试 fixture 仍按兼容层保留。云端默认 `WWIIHexV0Probes` 会覆盖数据启动、region / front / deployment、directive probe、灰潮 10 个 AI 半回合轻量运行时链路，以及 Playtest 红/蓝新局、Action Gate、主目标摘要、本地快照状态链路、neutral 主目标地面占领到即时胜利阈值的闭环、Modern Mission Recon 容器链路、Observer AI 容器入口自动推进、restricted / civilian fire ROE 规则入口和 direct attack contact gate；发布前仍需人工授权真实 App 运行时验证：iOS/macOS 启动、UI 点击烟测、SpriteKit 截图、真实 App 10-20 回合 observer 和性能体感。

本轮继续收口 fallback 显示：旧地名、旧 raw id 和旧编辑器生成区域名进入 Region inspector、Mission target summary、fire mission 叙事或选区反馈时，会显示为 `Objective Area`、`Zone Area`、`Sector Area` 或 `Contact Track` 等中性文案；AI provider、agent/context、command-chain 目标行和普通 diagnostics 也避免直接显示未知 provider id、raw region id、raw contact id、schema 细节或 pipeline 名称，但底层 region / theater / front-zone id、地图数据、provider id、审计 raw JSON 和命令目标不变。
...
- 元帅、国家约束、联合司令部、Chief of Staff、ISR / Fires / Air / EW / Logistics / Brigade roles 已进入当前指挥链，其中 ISR Recon Area 已有受限 `Command.recon` 执行桥，Fires Fire Mission 已有受限 `Command.fireMission` 执行桥，EW Electronic Warfare 已有受限 `Command.electronicWarfare` 执行桥，Air Tasking Suppress Air Defense 已新增 contact-gated 受限 `Command.suppressAirDefense` 执行桥
...
├── Probes/        — 云端默认 simulator probe target；本机默认不执行
...
- **v6.6 现代指挥链 advisory 层 + v6.10 受限执行桥**：`ModernCommandChainOrchestrator → ModernCommandChainPlan → ModernCommandChainDecoder`。它把元帅 `TheaterDirectiveEnvelope` 拆成国家约束、联合计划、Chief of Staff notes、ISR / Fires / Air / EW / Logistics / Brigade sub-directive，并校验 schemaVersion、issuerId、turn、faction、zone、region、contact 和 role/mission 组合；失败时只记录 diagnostics，不执行半成品。当前只有 ISR Recon Area、Fires Fire Mission、EW Electronic Warfare 和 Air Tasking Suppress Air Defense 四条受限 bridge 会最多编译 1 条 `Command`，并继续经 `CommandValidator -> RuleEngine` 执行。
...
| `Agents/ModernCommandChain.swift` | 现代指挥链 advisory 层与 v6.10 受限 command bridge | `StrategicConstraintEnvelope` / `JointCommandPlan` / `ModernSubDirective` / `ModernCommandChainDecoder` / `ModernCommandChainOrchestrator` / `ModernSubDirectiveCommandCompiler` |
...
| `UI/AgentPanelView.swift` | 决策展示 | 读 `record`（agent/provider/intent/context/command-chain replay/command results/errors；raw JSON 保留在记录层，普通面板默认不显示 Technical Replay） |
...
`MarshalAgent` 在成功解码 operational directive envelope 后生成 `ModernCommandChainPlan`，并把 fenced JSON 交给 `ModernCommandChainDecoder` 复核。decoder 会检查顶层和嵌套 envelope 的 schema、issuer、turn、faction、role，以及每条 sub-directive 的 zone / region / contact / mission 合法性；失败只写 diagnostics。`TurnManager` 将 Operational Directive JSON、Modern Command Chain JSON 和最终 Compiled ZoneDirective JSON 一起写入 `AgentDecisionRecord.rawJSON`；即使 operational 或 advisory JSON 校验失败，原始 JSON 也会尽量保留在记录层供审计。已验证的 sub-directive 会派生成 `ModernCommandChainReplayItem`，让 AI 面板直接列出 ISR / Fires / Air / EW / Logistics / Brigade 任务、目标和理由；普通玩家 AI 面板默认不显示 Technical Replay / raw JSON。v6.10 受限 bridge 目前只把 ISR Recon Area、Fires Fire Mission、EW Electronic Warfare 和 Air Tasking Suppress Air Defense 四类子任务各自最多编译 1 条 `Command`，执行结果进入 command results；Logistics / Brigade 仍保持 advisory / ZoneDirective 路径。
...
- 保持核心语义不退步；默认不在本机跑检查或重测试，静态检查、通用 iOS build 和 `WWIIHexV0Probes` 云端 simulator probe 由 GitHub Actions artifact 验证。
...
**你接手时的代码库状态：**
- v0.5 分支已引入元帅层与模拟 LLM JSON/decoder/ compiler；历史测试基线曾达到 v0.37 Probe 18/0、Stage Regression 69/0、Full 226/0。当前默认不在本机跑检查或重测试，静态检查、通用 iOS build 和 `WWIIHexV0Probes` 云端 simulator probe 看 GitHub Actions artifact。
...
push 到 `origin/main` 后核对 GitHub Actions artifact；旧测试口径残留、JSON / project / scheme / Ruby 脚本检查、通用 iOS build 和 `WWIIHexV0Probes` 云端 simulator probe 由 workflow 执行并写入结果包。未获人工授权时，不跑本机检查、完整 XCTest、历史 Stage / Full、UI / 截图或性能验证。
